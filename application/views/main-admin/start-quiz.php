...
<!-- One "custom_quiz_tab" for each quiz_step in the form: -->
		...
		$total_questions = ceil($total_questions/$per_page);
		$quiz_questions_limit = array_slice($quiz_questions,0,$per_page);
		$x = 1;
		...
		while($x <= $total_questions) {
			...
			<?php
			$offset = $per_page * $x;
			if(!empty($quiz_questions_limit)){
			foreach($quiz_questions_limit as $print){
			...
			$quiz_questions_limit = array_slice($quiz_questions,$offset,$per_page);
			$x++;
			?>
  </div>
  <?php
